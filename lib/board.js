...
class Board {
  ...
  findFloaters(smoot) {
    ...
    while (potentialCluster.length > 0) {
      ...
      const smootNeighborPositions = this.getSmootNeighborPositions(currentSmoot.gridPos);
  ...
  findNeighboringSmootMatches(smoot) {
    ...
    const smootNeighborPositions = this.getSmootNeighborPositions(smootGridPos);
  ...
  getSmootNeighborPositions(gridPos) {
    // gridPos: [rowIdx, slotIdx]
    ...
    gridDirections.forEach((direction) => {
      ...
      if (this.grid[neighborPos[0]][neighborPos[1]] instanceof Smoot) {
  ...
  hasReachedBottom() {
    return this.game.smoot.gridPos[0] >= Settings.BOARD.ROW_COUNT - 2;
  }
