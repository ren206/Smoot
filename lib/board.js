...
class Board {
  ...
  addRow() {
    ...
    for (let i = 0; i < Settings.BOARD.ROW_SIZE; i++) {
      let newSmoot;
    ...
    this.grid.forEach(row => {
      row.forEach(item => {
        if (item instanceof Smoot) {
          item.color = Settings.SMOOT_SPACE.COLOR;
          item.sad = true;
        }
      })
    })
  ...
  drawBottom(ctx) {
    ...
    ctx.fillRect(
      0, Settings.BOARD.HEIGHT - Settings.BOTTOM.HEIGHT,
  ...
  drawBottomLine(ctx) {
    ...
    ctx.beginPath();
    const lineHeight = Settings.BOTTM.HEIGHT;
  ...
  findFloaters(smoot) {
    ...
    while (potentialCluster.length > 0) {
      ...
      const neighborGridPositions = this.getNeighborGridPositions(currentSmoot.gridPos);
  ...
  getNeighborGridPositions(gridPos) {
    ...
    const leaningLeft = Boolean(gridPos[0] % 2 !== 0);
    ...
    gridDirections.forEach(direction => {
      ...
      if (
        Utils.betweenNums(neighborPos[0], 0, Settings.BOARD.ROW_COUNT) &&
        Utils.betweenNums(neighborPos[1], 0, Settings.BOARD.ROW_SIZE)
  ...
  getNeighborGridPositions2(gridPos) {
    const positions = [];
    const smootAtPos = this.grid[gridPos[0]][gridPos[1]];

    const gridDirections = Settings.BOARD.GRID_DIRECTIONS;
    let neighborPos;
    let neighborSmoot;
    gridDirections.forEach(direction => {
      neighborPos = [gridPos[0] + direction[0], gridPos[1] + direction[1]];
      neighborSmoot = this.grid[neighborPos[0]][neighborPos[1]];
      if(
        Utils.betweenNums(neighborPos[0], 0, Settings.BOARD.ROW_COUNT) &&
        Utils.betweenNums(neighborPos[1], 0, Settings.BOARD.ROW_SIZE) &&
        smootAtPos.isCollidedWith(neighborSmoot) &&
        positions.filter(pos => (pos[0] === neighborPos[0] && pos[1] === neighborPos[1])).length === 0
      ) positions.push(neighborPos);
    })
  ...
  hasReachedBottom() {
    let reachedBottom;
    this.grid[this.grid.length-1].forEach(item => {
      if (item instanceof Smoot) reachedBottom = true;
    })
