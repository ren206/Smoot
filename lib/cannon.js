...
class Cannon {
  constructor(game) {
    ...
    this.boreSize = {
      length: Settings.CANNON.LENGTH,
      width: Settings.CANNON.WIDTH
    }
  ...
  drawLineBore(ctx, {length, width}, {angle, mouseX, mouseY}) {
    ...
    ctx.beginPath();
    ctx.moveTo(
      centerX + 1.05 * length * Math.cos(angle),
      centerY - 1.05 * length * Math.sin(angle)
    );
    // ctx.strokeStyle = String(Int(this.game.smoot.color) + 10);
    ctx.lineTo(
      centerX + 1.1 * length * Math.cos(angle),
      centerY - 1.1 * length * Math.sin(angle)
    )
    ctx.stroke();
  }
