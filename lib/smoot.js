...
class Smoot {
  constructor(options) {
    ...
    this.color = options.color || Settings.COLORS[Math.floor(Math.random() * Settings.COLORS.length)];
    this.pos = options.pos;
    this.radius = options.radius || 25;
    this.vel = options.vel || {velX: 0, velY: 0};

    // this.collidedWithTop = this.collidedWithTop.bind(this);
  }
  ...
  move() {
    ...
    //   this.stop();
    // };
    if ((this.pos.centerX + this.radius >= Settings.WIDTH) || (this.pos.centerX <= this.radius)) {
